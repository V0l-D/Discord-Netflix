...
module.exports = class BrowserWindow extends Electron.BrowserWindow {
	...
	async checkNetflix() {
		...
		if (infos) {
			...
			if (match) avatar = crypto.createHash('md5').update(match[1]).digest('hex');

			// if the avatar doesn't show in the Rich Presence, it means it's not supported
			if (avatar) smallImageKey = crypto.createHash('md5').update(avatar).digest('hex'); //bad fix but who actually cares
